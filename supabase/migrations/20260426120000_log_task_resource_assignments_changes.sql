-- Audit changes to tasks.resource_assignments (JSON) in cm_change_logs via log_change.
...
BEGIN
  IF TG_OP = 'UPDATE' THEN
    ...
    IF to_jsonb(OLD.resource_assignments) IS DISTINCT FROM to_jsonb(NEW.resource_assignments) THEN
      PERFORM public.log_change(
        'task',
        NEW.id,
        'updated',
        'resource_assignments',
        COALESCE(OLD.resource_assignments::text, ''),
        COALESCE(NEW.resource_assignments::text, '')
      );
    END IF;
